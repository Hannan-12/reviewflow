...
export async function POST(req: Request) {
  try {
    const { name, email, company, phone, profiles, currentSetup, message } = await req.json()
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'GoHighReview <noreply@gohighreview.de>',
      to: 'hello@gohighreview.de',
      reply_to: email,
      subject: `Agency inquiry from ${name} — ${company}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: #F5C518; padding: 20px; border-radius: 8px 8px 0 0;">
            <h2 style="margin: 0; color: #1A1A1A;">New Agency Inquiry</h2>
            <p style="margin: 4px 0 0; color: #1A1A1A; opacity: 0.7; font-size: 14px;">Agency plan — custom quote request</p>
          </div>
          <div style="padding: 24px; background: #f9f9f9; border: 1px solid #e5e5e5; border-top: none; border-radius: 0 0 8px 8px;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr><td style="padding: 8px 0; color: #666; width: 160px;">Name</td><td style="padding: 8px 0; font-weight: bold; color: #111;">${name}</td></tr>
              ...
              ${profiles ? `<tr><td style="padding: 8px 0; color: #666;">No. of profiles</td><td style="padding: 8px 0; color: #111; font-weight: bold;">${profiles}</td></tr>` : ''}
              ${currentSetup ? `<tr><td style="padding: 8px 0; color: #666; vertical-align: top;">Current setup</td><td style="padding: 8px 0; color: #111;">${currentSetup}</td></tr>` : ''}
